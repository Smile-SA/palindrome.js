 ...
 */
export const getWeatherData = async function (baseUrl = 'https://api.open-meteo.com/v1/forecast') {
    let countries = [];
    ...
    for (let country of countries) {
        ...
        for (let city of cities) {
            if (country[city].latitude && country[city].longitude) {
                const url = `${baseUrl}?latitude=${country[city].latitude}&longitude=${country[city].longitude}&hourly=temperature_2m&current_weather=true&daily=temperature_2m_max,temperature_2m_min&timezone=UTC`;
                try {
